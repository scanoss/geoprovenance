...
show_help() {
  ...
  echo "   Setup and copy the required files into place on a server to run the SCANOSS GEO PROVENANCE API"
...
if [ "$FORCE_INSTALL" -eq 1 ]; then
  echo "[FORCE] Installing Geo Provenance API $ENVIRONMENT without prompts..."
else
...
cp "$SCRIPT_DIR/scanoss-geoprovenance-api.sh" /usr/local/bin || { echo "startup script copy failed"; exit 1; }
chmod +x /usr/local/bin/scanoss-geoprovenance-api.sh
...
if [ -f "$SCRIPT_DIR/$BINARY" ] ; then
  ...
  echo "Please copy the Geo Provenance API binary file into: /usr/local/bin/$BINARY"
fi
